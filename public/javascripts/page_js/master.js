var master = (function(){
	...
	_const.prototype = {
		...
		_start:function(){
			...
			if(objThis._identity != "visitor"){
				...
				layer.open({
					...
					shade: false,
					area: ['400px', '200px'], //宽高
					content: "<div style='margin:15px;'><span class='fui-user'></span>訪客，歡迎進入福智基金會網站。<br/><br/><br/>您可以選擇<a href='login.html'>登入</a>或<a href='registered.html'>註冊</a></span>。<br/><br/></div>",
		...
		_initialAll:function(){
			...
			objThis._time.append("HI~ XXX , 今天日期:" + nowTime.getFullYear() + "/" + (nowTime.getMonth()+1) + "/" + nowTime.getDate());
			...
			*/
			...
			objThis._index.on("click",$.proxy(function(event){
				if(objThis._identity == 'user')
				{
					location.href="master.html?identity=" + objThis._identity;
					
				}else if(objThis._identity == 'admin')
				{
					location.href = "master.html?identity=" + objThis._identity;
				}
			},this))
		...
		function logout(){
		location.href = "loading.html?condition=logout";
		}												